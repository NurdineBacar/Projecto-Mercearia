...
//		this.dataDaVenda = dataDaVenda;
	...
	public String toString3() {
		return codigo+";"+produto.getNomeDoProduto()+";"+produto.getCategoria()+";"+produto.getLote()+";"+quantidade+";"+valorAPagar;
	}
...
//			SimpleDateFormat simple= new SimpleDateFormat("dd/MM/yy");
	...
	public static void removerProduto(String nomeProduto,String lote,int quantidade) {
		try {
			ArrayList<Produto> produto= Bd.listarProdutos();
			for(int i=0;i<produto.size();i++) {
				if(nomeProduto.equals(produto.get(i).getNomeDoProduto())) {
	...
	public static void removerP(String codig) {
		
		 try{
			 ArrayList<Venda> listaVenda= Bd.listarVenda();
			 	int i=0,c=0;
				for(Venda listaV: listaVenda) {
					if(Integer.parseInt(codig)==listaV.getCodigo()) {
						c=i;
					}
					i++;
				}
				...
				listaVenda.remove(c);
				FileWriter ficheiro= new FileWriter("venda.txt");
				BufferedWriter write= new BufferedWriter(ficheiro);
				for(Venda listaV: listaVenda) {
					write.write(listaV.toString1());
					write.newLine();
				}
				write.close();
				ficheiro.close();
