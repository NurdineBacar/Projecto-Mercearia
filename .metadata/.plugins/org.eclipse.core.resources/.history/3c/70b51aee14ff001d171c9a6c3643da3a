...
public class Venda_UI extends JFrame {
	...
	public Venda_UI() {
		...
		tabelaVenda.setModel(new DefaultTableModel(
			Bd.tabelaVenda(),
			new String[] {
				"#", "NOME", "CATEGORIA", "QUANTIDADE", "VALOR A PAGAR"
			}
		));
		tabelaVenda.getColumnModel().getColumn(1).setPreferredWidth(110);
		tabelaVenda.getColumnModel().getColumn(3).setPreferredWidth(96);
		tabelaVenda.getColumnModel().getColumn(4).setPreferredWidth(125);
		
		JButton btnRemover = new JButton("REMOVER");
		btnRemover.setBackground(new Color(255, 255, 255));
		btnRemover.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				fila= tabelaVenda.getSelectedRow();
				int quantidade= Integer.parseInt(tabelaVenda.getValueAt(fila, 3).toString());
				
				Venda.removerProduto(tabelaVenda.getValueAt(fila, 1).toString(), tabelaVenda.getValueAt(fila, 1).toString(), quantidade);
				Venda.removerP(tabelaVenda.getValueAt(fila, 0).toString());
				tabelaVenda.setModel(new DefaultTableModel(
		...
		btnRemover.setBounds(403, 627, 192, 31);
		...
		btnEfectuar.setBounds(652, 627, 192, 31);
		...
		btnRefresh.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				close();
				new Venda_UI().setVisible(true);;
			}
		});
		btnRefresh.setBounds(165, 629, 192, 27);
